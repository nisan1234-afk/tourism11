...
- **שלב 5 — מפה/ציר-זמן/ניחוש-תמונה**: כל שלושת הרכיבים מכוסים, בגרסאות פרגמטיות:
  - **ציר זמן**: `mishor-hachof.html` — יפו-ת"א 1887→1909→1950 (הדוגמה המדויקת
    מ-CONTENT_GUIDE.md), כל אירוע מקושר לשאלת זיהוי קצרה (`.timeline-q`, מתוגברת
    אוטומטית ע"י `enhanceReviewQuestionsWithOpenAnswer`). מחלקות `.timeline*`
    ב-`style.css`, לשימוש חוזר בעתיד רק היכן שכרונולוגיה עצמה מלמדת (לא בכל נושא).
  - **מפה — "תכנן מסלול"**: `haamakim.html` — מפת מסלול אינטראקטיבית (`.route-map`)
    על שאלת רכבת העמק האמיתית (התלמיד/ה בוחר/ת 2 מ-4 תחנות אמיתיות, זה ממלא פתיח
    לתשובה הפתוחה הקיימת). **לא נבנה** widget דומה לשאלת "סיור יומיים מנהרייה"
    (מישור החוף) — היא שאלת תכנון פתוחה בלי סט-אתרים "נכון" יחיד, אז נשארה כתשובה
    פתוחה חופשית (הבוט הקיים כבר בודק אותה) — לכפות עליה UI של בחירה סגורה היה
    שקול להמצאת "תשובה נכונה" שלא קיימת במקור.
  - **מפה — "איפה זה?"**: כבר מכוסה מאז שלב 1 ע"י משחקי ההתאמה אתר↔תת-אזור
    (`SORT_ITEMS`) שקיימים בכל 4 דפי האזור.
  - **ניחוש מתמונה**: **לא בנוי בתוך tourism11 עצמו**, אבל מכוסה בפועל ע"י ריפו
    נפרד — `jerusalem-tour` (ר' סעיף למעלה) — 114 תמונות אמיתיות מ-4 חבלים,
    בפורמט כרטיסיות "הפוך וגלה שם", מחובר להתקדמות (ר' מסמך 07).
